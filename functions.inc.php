...
function paging_get_config($engine) {
	...
	case "asterisk":
		...
		foreach ($autoanswer_arr as $autosetting) {
			switch (freepbx_trim ($autosetting['var'])) {
			case 'ALERTINFO':
				$alertinfo = freepbx_trim ($autosetting['setting']);
				break;
			case 'CALLINFO':
				$callinfo = freepbx_trim ($autosetting['setting']);
				break;
			case 'SIPURI':
				$sipuri = freepbx_trim ($autosetting['setting']);
				break;
			case 'VXML_URL':
				$vxml_url = freepbx_trim ($autosetting['setting']);
				break;
			case 'DOPTIONS':
				$doptions = freepbx_trim ($autosetting['setting']);
				break;
			case 'DTIME':
				$dtime = freepbx_trim ($autosetting['setting']);
				break;
			default:
				$key = freepbx_trim ($autosetting['var']);
				$custom_vars[$key] = freepbx_trim ($autosetting['setting']);
...
// dial all number FREEPBX-14551 intercom Override in options
		...
		foreach ($autoanswer_arr as $autosetting) {
			$useragent   = freepbx_trim ($autosetting['useragent']);
			$autovar     = freepbx_trim ($autosetting['var']);
			$data        = freepbx_trim ($autosetting['setting']);
		...
		$ext->add($macro, "s", '', new ext_return());
		...
		foreach ($paging_groups as $thisgroup) {
			$grp=freepbx_trim ($thisgroup['page_group']);
...
function paging_getdestinfo($dest) {
	if (substr(freepbx_trim ($dest),0,15) == 'app-pagegroups,') {
		$exten = explode(',',$dest);
		$exten = $exten[1];
...
function paging_configpageload() {
	...
	if ($action != 'del' && $tech_hardware != 'virtual') {
		...
		$answermode = (freepbx_trim ($answermode) == '') ? $amp_conf['DEFAULT_INTERNAL_AUTO_ANSWER'] : $answermode;
		...
		$intercom = (freepbx_trim ($intercom) == '') ? 'enabled' : $intercom;
